...
def _ensure_2d_windows(x: np.ndarray) -> np.ndarray:
    # Accept [N,T,1] or [N,T] -> return [N,T]
...
def _ks_statistic(x: np.ndarray, y: np.ndarray) -> float:
    # Two-sample KS D statistic (no p-value), numpy only
...
def parse_args():
    ...
    # Where sample_diffusion_vae_cond.py writes its vixXX_wY folders
    p.add_argument(
        "--gen_root",
        type=str,
        default="experiments/results/diffusion_latent_cond_sp500_logret_L50",
    )
    p.add_argument("--out_root", type=str, default="experiments/results/eval_cond_sp500_L50")
    p.add_argument("--min_n_real_matched", type=int, default=200)
...
def main():
    ...
    real_x_test, real_c_test, c_stats = _load_real_test(win_dir=win_dir, seq_len=seq_len, name=name)

    if not gen_root.exists():
        raise FileNotFoundError(
            f"Missing {gen_root}. Did you run scripts/sample_diffusion_vae_cond.py?"
        )

    vix_dirs = sorted([p for p in gen_root.iterdir() if p.is_dir() and p.name.startswith("vix")])
    if not vix_dirs:
        raise ValueError(f"No vixXX subfolders found in {gen_root}")
    ...
    for d in vix_dirs:
        try:
            # folder looks like: vix30_w2.0 or vix30_w2
        ...
        real_subset = _real_subset_by_vix(
            ...
            min_n=int(args.min_n_real_matched),
        )
    ...
    for r in deltas_rows:
        print(
            ...
            f"mdd_gap_mean={r['mdd_gap_mean']:.3f} | acf1_gap={r['acf1_gap']:.3f}"
        )
